...
public class ProductPage2 {
	...
	public ProductPage2(WebDriver driver) {
		...
		// fluent wait allows wait us periodically
		
		wait = new FluentWait<>(driver).withTimeout(Duration.ofSeconds(20)).//max amount to wait for cond to met
				pollingEvery(Duration.ofSeconds(2))//checks every 2 secs
	...
	public void selectColour() {
		WebElement colourOption = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(@class, 'swatch-option color')]")));
		colourOption.click();
	}
	...
	public void addToCart() {
		...
		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class,'message-success')]")));
		System.out.println("Product added to cart!");
	}

	// Open Cart
	public void openCart() {
		WebElement myCart = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[@class='action showcart']")));
		myCart.click();
	}
	...
	public String getCartProductName() {
		WebElement productName = wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='product-item-details']//a")));
		return productName.getText();
	}

	//
	public void viewAndEditCart() {
		WebElement viewEditCartBtn = wait
				.until(ExpectedConditions.elementToBeClickable(By.linkText("View and Edit Cart"))); // Locate "View and
																									// Edit Cart"
